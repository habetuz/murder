...
public class AuthenticatorBuilder
{
    ...
    public AuthenticatorBuilder Register<TCredential, TEnrollmentData, TMethod>(
        IAuthenticationMethod<TCredential, TEnrollmentData, TMethod> method
    )
        ...
        where TMethod : IAuthenticationMethodKey
    {
        _methods.Add(
            typeof(TMethod),
            new AuthenticationMethodAdapter<TCredential, TEnrollmentData, TMethod>(method)
        );
        return this;
    }
    ...
    private sealed class AuthenticationMethodAdapter<TCredential, TEnrollmentData, TMethod>(
        IAuthenticationMethod<TCredential, TEnrollmentData, TMethod> method
        ...
        where TMethod : IAuthenticationMethodKey
    {
        ...
        public IdentityId? Authenticate(object credential)
        {
            if (credential is not TCredential typedCredential)
                throw new AuthenticationMethodMismatchException(
                    typeof(TCredential),
                    credential.GetType()
                );
        ...
        public object Enroll(object enrollmentData)
        {
            if (enrollmentData is not TEnrollmentData typedEnrollmentData)
                throw new AuthenticationMethodMismatchException(
                    typeof(TEnrollmentData),
                    enrollmentData.GetType()
                );
